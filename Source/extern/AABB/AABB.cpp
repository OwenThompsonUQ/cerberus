/*
  ...
  Copyright (c) 2020 Daryl Bond <daryl_bond@hotmail.com>
  ...
  This code was adapted from parts of the aabbcc library by Lester Hedges
...
double point2triangleDistance(const double *pt, const double *tri_0,
                              const double *tri_1, const double *tri_2) {
  BL_PROFILE("point2triangleDistance");

  double diff, edge0, edge1;
  double a00, a01, a11, b0, b1, c;

  for (int i = 0; i < 3; ++i) {
    diff = tri_0[i] - pt[i];
    edge0 = tri_1[i] - tri_0[i];
    edge1 = tri_2[i] - tri_0[i];
...
int point2triangleSide(const double *pt, const double *tri_0,
                       const double *tri_1, const double *tri_2) {
  BL_PROFILE("point2triangleSide");

  double edge0[3], edge1[3], n[3], v[3];

  for (int i = 0; i < 3; ++i) {
    edge0[i] = tri_1[i] - tri_0[i];
    edge1[i] = tri_2[i] - tri_0[i];
    v[i] = pt[i] - tri_0[i];
  }

  n[0] = -edge0[2] * edge1[1] + edge0[1] * edge1[2];
  n[1] = edge0[2] * edge1[0] - edge0[0] * edge1[2];
  n[2] = -edge0[1] * edge1[0] + edge0[0] * edge1[1];
...
double AABB::getSurfaceArea() const { return surfaceArea; }
...
double AABB::computeRadius() {
  double d, r = 0;
  for (unsigned int i = 0; i < centre.size(); ++i) {
    d = upperBound[i] - centre[i];
    r += d * d;
  }
...
std::vector<double> AABB::closestPoint(const std::vector<double> &p) const {
  BL_PROFILE("AABB::closestPoint()");
  ...
  for (int i = 0; i < dim; i++) {
    double v = p[i];
    if (v < lowerBound[i])
      v = lowerBound[i]; // v = max( v, b.min[i] )
    if (v > upperBound[i])
      v = upperBound[i]; // v = min( v, b.max[i] )
    q[i] = v;
  }
...
bool AABB::isInside(const std::vector<double> &p) const {
  BL_PROFILE("AABB::closestPoint()");
...
double AABB::closestDistance(const std::vector<double> &xyz) const {
  BL_PROFILE("AABB::closestDistance()");

  std::vector<double> q = closestPoint(xyz);

  // calculate the squared distance
  double sd = 0.0;
  for (int d = 0; d < xyz.size(); ++d) {
    q[d] -= xyz[d];
    sd += q[d] * q[d];
  }
...
Sphere::Sphere(const std::vector<double> &c, double r) : centre(c), radius2(r) {
  radius2 *= radius2;
}

bool Sphere::overlaps(const AABB &aabb, bool touchIsOverlap) const {
  BL_PROFILE("Sphere::overlaps()");

  std::vector<double> q = aabb.closestPoint(centre);

  // calculate the squared distance
  double sd = 0.0;
  for (int d = 0; d < centre.size(); ++d) {
    q[d] -= centre[d];
    sd += q[d] * q[d];
  }

  if ((sd < radius2) || ((sd == radius2) && touchIsOverlap)) {
    return true;
  } else {
    return false;
  }
...
bool Node::isLeaf() const { return (left == NULL_NODE); }

Tree::Tree() {}

void Tree::init(unsigned int dimension_, double skinThickness_,
                unsigned int nParticles, bool touchIsOverlap_) {
...
void Tree::insertParticle(unsigned int particle, std::vector<double> &position,
                          double radius) {
...
void Tree::insertParticle(unsigned int particle,
  ...
  BL_PROFILE("Tree::insertParticle()");
...
unsigned int Tree::nParticles() { return particleMap.size(); }
...
bool Tree::updateParticle(unsigned int particle, std::vector<double> &position,
                          double radius, bool alwaysReinsert) {
  // Validate the dimensionality of the position vector.
  if (position.size() != dimension) {
    throw std::invalid_argument("[ERROR]: Dimensionality mismatch!");
  }
...
bool Tree::isInside(const std::vector<double> &xyz) const {
  return nodes[root].aabb.isInside(xyz);
}

double Tree::signedDistance(const std::vector<double> &xyz,
                            const std::vector<std::array<double, 9>> &tris) {
  BL_PROFILE("Tree::signedDistance()");

  // make a test sphere that overlaps the whole domain

  Sphere sphere(xyz, std::numeric_limits<double>::max());

  // now do the tree traversal
  ...
  while (stack.size() > 0) {
    ...
    // Test for overlap between the AABB and the sphere.
    if (sphere.overlaps(nodeAABB, touchIsOverlap)) {
      // Check that we're at a leaf node.
      if (nodes[node].isLeaf()) {
        const std::array<double, 9> &facet = tris[nodes[node].particle];
        double rs = point2triangleDistance(sphere.centre.data(), &facet[0],
                                           &facet[3], &facet[6]);

        // catch any floating point errors
        // (moved here as optimization in point2triangleDistance function just
        // returns 0.0 !!??!!)
        rs = std::max(rs, 0.0);

        if (rs < min_distance) {
          min_distance = rs;
          min_particle = nodes[node].particle;
          sphere.radius2 = rs;
        }
        tri_touched++;
      } else {
        // check which AABB is closer and push that one to the back of the stack
        AABB &L = nodes[nodes[node].left].aabb;
        AABB &R = nodes[nodes[node].right].aabb;

        if (L.closestDistance(xyz) < R.closestDistance(xyz)) {
          stack.push_back(nodes[node].right);
          stack.push_back(nodes[node].left);
        } else {
          stack.push_back(nodes[node].left);
  ...
  // get the signed side on which the sample point lies
  int side = point2triangleSide(sphere.centre.data(), &tris[min_particle][0],
                                &tris[min_particle][3], &tris[min_particle][6]);

  return side * std::sqrt(min_distance);
}
...
unsigned int Tree::computeHeight() const { return computeHeight(root); }
...
unsigned int Tree::getNodeCount() const { return nodeCount; }
